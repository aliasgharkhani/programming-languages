...
(define reverse
  (lambda (a)
    (cond
      [(null? a) a]
      [else (append (reverse (cdr a)) (list (car a)))]
      )
    ))
...
(define reverse_all
  (lambda (a)
    (cond
      [(null? a) a]
      [else (let ((head (car a)) (tail (cdr a))) (append (reverse_all tail) (cond
                                                                              [(list? head) (list (reverse_all head))]
                                                                              [else (list head)]) ))]
      )
    ))
...
(define set
  (lambda (a index value)
    (cond
      [(null? a) "list index outof range"]
      [(eq? index 0) (cons value (cdr a))]
      [else (let ((aux (set (cdr a) (- index 1) value))) (if (eq? aux "list index outof range")
                                                             "list index outof range"
                                                             ((cons (car a) aux))))])
    ))
